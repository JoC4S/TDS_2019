...
def SampleFilter_get():
    ...
    accX = (accX / pow(2, 16))*1.925
    accY = (accY / pow(2, 16))*1.925
    accZ = (accZ / pow(2, 16))*1.925
...
def angulos():
    global angX, accX, accY, accZ, angY
    if (accX > 1):
        accX = 1
    elif (accX < -1):
        accX = -1
    if (accY > 1):
        ...
        accY = -1
    angY = math.degrees(math.atan(accY/(math.sqrt(pow(accX,2)+pow(accZ,2)))))
    if accZ > 0 and accY < 0:
        angY = angY * -1
    if accZ < 0 and accY < 0:
        angY = 180 - abs(angY)
    if accZ < 0 and accY > 0:
    ...
    angX = math.degrees(math.atan(accX/(math.sqrt(pow(accY,2)+pow(accZ,2)))))
    if accZ > 0 and accX < 0:
        angX = angX * -1
    if accZ < 0 and accX < 0:
        angX = 180 - abs(angX)
    if accZ < 0 and accX > 0:
    ...
    #angX = math.degrees(math.acos(accX))
    #angY = math.degrees(math.acos(accY))
...
def Dataget (tiempo):
    ...
    data = imu.getIMUData()
    # e obtiene el dato de aceleracion en la variable accel
    accel = data["accel"]
    Gx = accel[0]
    Gy = accel[1]
    Gz = accel[2]
